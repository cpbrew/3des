...
char *encrypt(uint64_t, char *, int);
char *decrypt(uint64_t, char *, int);
uint64_t *deriveRoundKeys(uint64_t);
uint64_t permutate(uint64_t, int, const int *, int);
uint64_t desBlock(uint64_t *, uint64_t);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
        ...
        buffer = new char[fileSize + padding];
        ...
        buffer = encrypt(keys[0], buffer, fileSize + padding);

        f.open(argv[4], ios::out | ios::binary);
        f.write(buffer, fileSize + padding);
        f.close();
    }
        ...
        buffer = decrypt(keys[0], buffer, fileSize);
...
char *encrypt(uint64_t key, char *cleartext, int length) {
    uint64_t *buffer = new uint64_t[length / 8];
    ...
    for (int i = 0; i < (length / 8); i++) {
        ...
        for (int j = 0; j < 8; j++) {
            block <<= 8;
            block |= cleartext[(i * 8) + j];
        }
        buffer[i] = desBlock(roundKeys, block);
    }
...
char *decrypt(uint64_t key, char *ciphertext, int length) {
    uint64_t *buffer = new uint64_t[length / 8];
    uint64_t *roundKeys = deriveRoundKeys(key);
    uint64_t *reversedKeys = new uint64_t[16];
    for (int i = 0; i < 16; i++) {
        reversedKeys[16 - i - 1] = roundKeys[i];
    }
    ...
    for (int i = 0; i < (length / 8); i++) {
        ...
        for (int j = 0; j < 8; j++) {
            block <<= 8;
            block |= ciphertext[(i * 8) + j];
        }
        buffer[i] = desBlock(reversedKeys, block);
    }

    char *cleartext = new char[length];
    for (int i = 0; i < (length / 8); i++) {
        uint64_t n = buffer[i];
        for (int j = 7; j >= 0; j--) {
            cleartext[(i * 8) + j] = (char) (n & 0xFF);
            n >>= 8;
        }
    }
...
uint64_t desBlock(uint64_t *roundKeys, uint64_t block) {
